...
Pagamenti abbonamento barbiere: Stripe Billing (stripe@20.3.1, API 2026-01-28.clover). 3 piani: Essential €300/mese (prod_TwyoUI0JLvWcj3), Professional €500/mese (prod_TwypWo5jLd3doz), Enterprise custom (prod_TwyphvT1F82GrB). Trial 7gg. Stripe Checkout con selezione piano, setup fee €500 one-time (prod_Txi5JcLgAyUgxl) come secondo line_item, e codici promozionali/coupon (allow_promotion_codes). Stripe Webhook /api/stripe/webhook: checkout.session.completed per setup fee (match su product ID, setta setup_fee_paid + setup_fee_paid_at), invoice.paid per referral reward (€50 credito via Customer Balance al referrer). Stripe Customer Portal per self-service. Nessun pagamento del cliente finale nell'MVP. Sistema referral: codice unico per business (REF-NOME-XXXX), €50 credito al referrer per ogni invitato che si abbona, 20% sconto primo mese all'invitato.

Cron e job scheduling: pg_cron v1.6.4 nativo di Supabase per job ricorrenti (conferma intelligente, review, riattivazione, analytics). pg_net v0.19.5 per chiamate HTTP async alle Edge Functions. 8 cron schedules attivi. Trigger SQL trg_recalc_analytics su appointments per aggiornamento real-time di analytics_daily ad ogni cambio stato/data/servizio (cron notturno come safety net).
